...
public class MaterialUploadImageCmdExe extends AbstractCmdExe<MaterialUploadCmd, SingleResponse> {
    ...
    protected SingleResponse handle(MaterialUploadCmd cmd) {
        ...
        try {
            ...
            if ("mp4".equalsIgnoreCase(suffix) || "video".equalsIgnoreCase(suffix)){
            ...
            if ("minio".equalsIgnoreCase(ossDefault)) {
                uploadVO = MinioSender.uploadFileEx(materialUploadCO.getFile(), minioOssConfig.toConfig(), cmd.getUserAware().getMchId() + "", imageType.name());
            } else if ("upYun".equalsIgnoreCase(ossDefault)) {
                uploadVO = UpYunSender.uploadFileEx(materialUploadCO.getFile(), upYunConfig, cmd.getUserAware().getMchId() + "", imageType.name());
            }
            ...
            return SingleResponse.buildFailure("-99", "上传图片失败");
        }
        return SingleResponse.buildFailure("-99", "上传图片失败");
    }
