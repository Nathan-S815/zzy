...
@Slf4j
@Component
public class OnAppCmdExe extends AbstractCmdExe<OnAppCmd, SingleResponse> {

    @Autowired
    private AppInfoService appInfoService;

    @Override
    protected SingleResponse handle(OnAppCmd cmd) {
        boolean update = appInfoService.lambdaUpdate().eq(AppInfo::getId, cmd.getId()).set(AppInfo::getStatus, 1).update();
        if (update) {
