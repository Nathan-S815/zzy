...
@Slf4j
@Component
public class ModifyVersionCmdExe extends AbstractCmdExe<ModifyVersionCmd, SingleResponse> {
    ...
    @Override
    protected SingleResponse handle(ModifyVersionCmd cmd) {
        platUpgradeService.lambdaUpdate()
                .set(PlatUpgrade::getTitle, cmd.getTitle())
                .set(PlatUpgrade::getUpgradeDate, cmd.getUpgradeDate())
                .set(PlatUpgrade::getVersion, cmd.getVersion())
                .eq(PlatUpgrade::getId, cmd.getId())
                .update();
        platUpgradeDetailsService.remove(new LambdaQueryWrapper<PlatUpgradeDetails>().eq(PlatUpgradeDetails::getPlatUpgradeId,cmd.getId()));
        ...
        List<PlatUpgradeDetails> details = cmd.getItems().stream().map(x -> {
            ...
            platUpgradeDetails.setPlatUpgradeId(cmd.getId());
