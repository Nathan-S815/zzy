package com.nuwa.attract.start.api.aop.annotation;
...
public class JWTUtil {
     ...
     * @return 加密的token
     */
    public static String sign(String username, Long userId, Integer accountType) {
        ...
        return JWT.create()
            ...
            .withClaim(ACCOUNT_TYPE, accountType)
     ...
     * 获得token中的信息，无需secret解密也能获得
     */
    public static String getMchName(String token) {
        try {
            DecodedJWT jwt = JWT.decode(token);
            Claim claim = jwt.getClaim(MCH_NAME);
            if (BeanUtil.isNotEmpty(claim)) {
                return claim.asString();
            }
            return null;
