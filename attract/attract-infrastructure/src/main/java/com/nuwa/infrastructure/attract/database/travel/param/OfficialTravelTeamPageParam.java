...
@ApiModel(value = "分页参数")
public class OfficialTravelTeamPageParam extends PageQry<TravelTeam> {
    private static final long serialVersionUID = 1L;

    private OfficialTravelTeamPageQry qry;

    public OfficialTravelTeamPageParam(OfficialTravelTeamPageQry qry) {
        super(qry);
        this.qry = qry;
    }
    ...
    public LambdaQueryWrapper<TravelTeam> toQueryWrapper() {
        ...
        queryWrapper.eq(Objects.nonNull(qry.getApplyTime()), TravelTeam::getApplyTime, qry.getApplyTime());
        ...
        if (Objects.nonNull(qry.getStatus())) {
            if (qry.getStatus().equals(1)) {
                queryWrapper.eq(TravelTeam::getTeamStatus, TeamStatusEnum.WAIT_OFFICIAL.getCode());
            } else if (qry.getStatus().equals(2)){
                queryWrapper.eq(TravelTeam::getTeamStatus, TeamStatusEnum.OFFICIAL_SUCCESS.getCode());
            } else {
                queryWrapper.eq(TravelTeam::getTeamStatus, TeamStatusEnum.OFFICIAL_REJECT.getCode());
            }
        } else {
