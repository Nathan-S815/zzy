...
public enum ErrorEnum implements ErrorCodeI {
    ...
    TRADE_NOT_EXIST("700", "订单不存在"),
    ORDER_STATUS_ERROR("701", "订单状态错误"),
    CREATE_ORDER_FAILED("702", "创建订单失败"),
    REFUND_ORDER_FAILED("703", "退款失败"),
    REPEAT_PRODUCT("704", "商品重复"),
    MISSING_REQUIRED_PARAMS("705", "缺少必要参数"),
    STOCK_FAILED("706", "下单失败"),
    PRODUCT_NOT_EXIST("707", "商品不存在"),
    NOT_ENOUGH("708", "库存不足"),
    ACTIVITY_NOT_EXIST("709", "活动不存在"),
    APPLY_FAILED("710", "报名失败"),
    REPEAT_APPLY("711", "请勿重复报名"),
    ACTIVITY_TICKETS_NOT_ENOUGH("712", "余票不足"),
    ACTIVITY_ADD_FAILED("713", "活动新增失败"),
    DATA_NON("714", "暂无数据"),
    DEL_ONLINETYPE_FAILED("715", "请先删除类别下的类型"),
    ...
    public SingleResponse build() {
        if (SUCCESS.getErrCode().equals(this.getErrCode())) {
            return buildFailure();
        } else {
            return buildSuccess();
        }
