...
import com.nuwa.client.ticket.dto.clientobject.activity.OffActivityCategoryCmd;
...
@Slf4j
@Component
public class OffActivityCategoryCmdExe extends AbstractCmdExe<OffActivityCategoryCmd, SingleResponse> {

    @Autowired
    private CultureActivityCategoryService cultureActivityCategoryService;

    @Override
    protected SingleResponse handle(OffActivityCategoryCmd cmd) {
        ...
        LambdaUpdateChainWrapper<CultureActivityCategory> lambdaUpdate = cultureActivityCategoryService.lambdaUpdate()
                .set(CultureActivityCategory::getPublishStatus, PublishStatusEnum.OFF.getCode())
