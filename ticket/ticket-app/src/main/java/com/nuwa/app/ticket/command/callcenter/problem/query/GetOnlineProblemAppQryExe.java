...
@Slf4j
@Component
public class GetOnlineProblemAppQryExe extends AbstractQryExe<GetOnlineProblemAppCmd, SingleResponse> {
    @Autowired
    ...
    protected SingleResponse handle(GetOnlineProblemAppCmd cmd) {
        List<OnlineProblem> list = onlineProblemService.lambdaQuery()
                ...
                .eq(OnlineProblem::getCategory, cmd.getCategory())
                .eq(OnlineProblem::getType, cmd.getType())
//                .eq(OnlineProblem::getAppId,cmd.getUserAware().getMchAppId())
                .eq(OnlineProblem::getMchId,cmd.getUserAware().getMchId())
                .eq(!StrUtil.isBlankOrUndefined(cmd.getProblem()),OnlineProblem::getProblem, cmd.getProblem()).list();
        if(list.size()>0) {
