...
@Slf4j
@Component
public class UserMallProductQryExe extends AbstractQryExe<UserMallProductQry, SingleResponse> {
    ...
    @Override
    protected SingleResponse handle(UserMallProductQry cmd) {
        if (Objects.isNull(cmd.getId())) {
            return ErrorEnum.MISSING_REQUIRED_PARAMS.buildFailure();
        }
        MallProductQryExe.ProductVo vo = new MallProductQryExe.ProductVo();
