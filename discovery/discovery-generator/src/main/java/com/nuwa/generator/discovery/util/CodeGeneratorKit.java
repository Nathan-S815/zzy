...
package com.nuwa.generator.discovery.util;
...
import com.nuwa.generator.discovery.config.*;
import com.nuwa.generator.discovery.config.query.SpringBootPlusMySqlQuery;
import com.nuwa.generator.discovery.config.query.SpringBootPlusSqlServerQuery;
...
public class CodeGeneratorKit {
    ...
    public void generator(GeneratorProperties generatorProperties) {
        ...
        if (tableConfig.isEmpty()) {
            throw new RuntimeException("tableConfig不能为空");
        }
    ...
    public void init(String tableName, GeneratorProperties generatorProperties) {
        ...
        if (StringUtils.isBlank(mavenModuleName)) {
            throw new RuntimeException("mavenModuleName不能为空");
        }
    ...
    public void generator(String tableName, String pkIdName, GeneratorProperties generatorProperties) {
        ...
        if (mybatisPlusGeneratorConfig == null) {
            throw new RuntimeException("mybatisPlusGeneratorConfig不能为空");
        }

        // 全局配置
        GlobalConfig globalConfig = mybatisPlusGeneratorConfig.getGlobalConfig();
        if (globalConfig == null) {
            throw new RuntimeException("globalConfig不能为空");
        }
        ...
        globalConfig.setActiveRecord(generatorProperties.getGeneratorConfig().isActiveRecord());
        ...
        if (dataSourceConfig == null) {
            throw new RuntimeException("dataSourceConfig不能为空");
        }
        ...
        if (strategyConfig == null) {
            throw new RuntimeException("strategyConfig不能为空");
        }
        // 设置生成的表
        strategyConfig.setInclude(tableName);
        // 表前缀
        List<String> tablePrefixList = generatorProperties.getTablePrefix();
        if (tablePrefixList.isEmpty()) {
        ...
        InjectionConfig injectionConfig = new InjectionConfig() {
            ...
            public void initMap() {
                ...
                String pageQryParentPackage = generatorProperties.getPageQryPackage();
                ...
                String entityPackage = projectConfig.getEntityPackage();
                ...
                String pageQryPackage = pageQryParentPackage + finalModuleName + StringPool.DOT + GeneratorConstant.Qry + StringPool.DOT + entityName + "PageQry";
                map.put("pageQryPackage", pageQryPackage);

                map.put("entityPackage", entityPackage + StringPool.DOT + GeneratorConstant.SUPER_ENTITY_CLASS + entityName);
                map.put("superPageQryPackage", superPageOrderParamClass);

                // 查询参数类路径
                map.put("pageParamClass", pageParamPackage + StringPool.DOT + entityName);
                // 查询参数共公包路径
                ...
                String queryPagePackage = parentPackage + finalModuleName + StringPool.DOT + GeneratorConstant.Qry;
                map.put("queryPagePackage", queryPagePackage);

                map.put("superQueryCOClass", projectConfig.getSuperQueryCOClass());
                map.put("queryCOPackage", parentPackage + finalModuleName);
        ...
        // 自定义pageParam模板
 /*       if (generatorConfig.isGeneratorPageQry()) {
            focList.add(new FileOutConfig(GeneratorConstant.QRY_PAGE_TEMPLATE_PATH) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return finalJavaOutputDir + File.separator + parentPackagePath + finalModulePath + File.separator + "qry" +
                            File.separator + tableInfo.getEntityName() + GeneratorConstant.PAGE_PARAM_QRY + StringPool.DOT_JAVA;
                }
            });
        }*/

        // 自定义pageParam模板GeneratorPageParam
        if (generatorConfig.isGeneratorPageParam()) {
            focList.add(new FileOutConfig(GeneratorConstant.PAGE_QRY_TEMPLATE_PATH) {
                @Override
                public String outputFile(TableInfo tableInfo) {
        ...
        if (generatorConfig.isGeneratorPageQry()) {
            focList.add(new FileOutConfig(GeneratorConstant.QRY_PAGE_TEMPLATE_PATH) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return finalJavaOutputDir + File.separator + parentPackagePath + finalModulePath +
                            File.separator + "qry" + File.separator + tableInfo.getEntityName() + GeneratorConstant.PAGE_PARAM_QRY + StringPool.DOT_JAVA;
                }
            });
        }
